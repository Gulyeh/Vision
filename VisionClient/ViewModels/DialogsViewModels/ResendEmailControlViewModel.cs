...
{
    internal class ResendEmailControlViewModel : DialogHelper
    {
        ...
        public ResendEmailControlViewModel(IEventAggregator eventAggregator, IAccountRepository accountRepository,
        ...
        public override void OnDialogOpened(IDialogParameters parameters) => eventAggregator.GetEvent<SendEvent<string>>().Publish("ShadowLoginWindow");

        public override void OnDialogClosed() => eventAggregator.GetEvent<SendEvent<string>>().Publish("ShadowLoginWindow");
        ...
        protected override async void Execute(object? data)
        {
            try
            {
                ...
                (bool isSuccess, string? response) = await accountRepository.ResendEmailConfirmation(Email);
                if (!isSuccess)
                {
